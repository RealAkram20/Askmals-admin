@extends('layouts.seller.app',['page' => $menuSeller['ad_campaigns']['active'] ?? "", 'sub_page' => $menuSeller['ad_campaigns']['route']['dashboard']['sub_active'] ?? "" ])
...
@section('seller-content')
    <div class="row row-deck row-cards" id="ad-dashboard">
                ...
                <div class="card stat-card">
                    <div class="card-body">
                        <div class="d-flex align-items-center">
                            <div class="stat-icon bg-azure-lt text-azure me-3">
                                <i class="ti ti-wallet fs-2"></i>
                            </div>
                            <div>
                                <div class="stat-value" id="statWallet">{{ $data['wallet']['formatted_balance'] }}</div>
                                <div class="stat-label">{{ __('labels.ad_dashboard_wallet_balance') }}</div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        {{-- Budget utilization radial bar --}}
        <div class="col-lg-4">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">{{ __('labels.ad_dashboard_budget_utilization') }}</h3>
                </div>
                <div class="card-body">
                    <div id="budgetRadialChart" style="height: 280px;"></div>
                </div>
            </div>
        </div>
                        ...
                        <table class="table table-vcenter card-table" id="topCampaignsTable">
                            ...
                            <tbody id="topCampaignsBody">
                                ...
                                @if(empty($data['top_campaigns']))
                                    <tr>
                                        <td colspan="8" class="text-center text-muted py-4">{{ __('labels.no_data_available') }}</td>
                                    </tr>
                                @endif
                            </tbody>
                        </table>
...
@push('scripts')
    ...
    <script>
        window.adDashboardConfig = {
            dataUrl: '{{ route("seller.ads.campaigns.dashboard.data") }}',
            initialData: @json($data),
            currencySymbol: '{{ $systemSettings["currencySymbol"] ?? "$" }}',
            panel: 'seller',
        };
    </script>
    <script src="{{ hyperAsset('assets/js/ad-dashboard-seller.js') }}" defer></script>
@endpush
