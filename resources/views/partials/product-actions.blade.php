<div class="d-flex justify-content-start align-items-center">
    ...
    @if(($showBadgeAction ?? false) && ($editPermission ?? false))
        <a
            href="javascript:void(0);"
            class="btn {{ ($badge ?? null) ? 'btn-rss' : 'btn-outline-rss' }} me-2 p-1"
            data-bs-toggle="modal"
            data-bs-target="#assign-badge-modal"
            data-id="{{ $id }}"
            data-title="{{ $title }}"
            data-badge-id="{{ $badge->id ?? '' }}"
            title="{{ __('labels.assign_badge') }}"
        >
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="{{ ($badge ?? null) ? 'currentColor' : 'none' }}"
                 stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                 class="icon icon-tabler icons-tabler-outline icon-tabler-badge m-0">
