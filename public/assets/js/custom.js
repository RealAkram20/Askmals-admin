...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('form.form-submit').forEach(function (form) {
        form.addEventListener('submit', function (e) {
            ...
            const redirectFromForm = form.dataset.redirect;
...
document.addEventListener('click', function (event) {
    ...
    const toggleStoreRecommended = event.target.closest('.toggle-store-recommended');
    if (!toggleStoreRecommended) return;

    const id = toggleStoreRecommended.getAttribute('data-id');

    toggleStoreRecommended.disabled = true;

    const originalHtml = toggleStoreRecommended.innerHTML;
    toggleStoreRecommended.innerHTML = `
        <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
    `;

    axios.post(`${base_url}/${panel}/sellers/store/${id}/toggle-recommended`)
        .then(function (response) {
            const data = response.data;
            ...
            toggleStoreRecommended.disabled = false;
            toggleStoreRecommended.innerHTML = originalHtml;
        })
        .catch(function () {
            Toast.fire({
                icon: 'error', title: 'An error occurred while updating recommendation.'
            });
        ...
        if (btnFilter) {
            btnFilter.addEventListener('click', function () {
                ...
                storesTable.settings()[0].ajax.data = function (d) {
                    ...
                    d.recommended_status = $('#recommended-status').val();
                };
    ...
    if (selectProduct) {
        ...
        window.TomSelect && new TomSelect((el = document.getElementById('select-product')), {
            ...
            load: function (query, callback) {
                ...
                const sellerEl = document.getElementById('select-seller');
                const sellerId = sellerEl && sellerEl.tomselect ? sellerEl.tomselect.getValue() : '';
                if (sellerId) {
...
document.addEventListener("DOMContentLoaded", function () {
    if (window.TomSelect) {
        document.querySelectorAll('.seo-keywords-select').forEach((sel) => {
            if (!sel.tomselect) {
                new TomSelect(sel, {
                    create: true,
                    persist: false,
...
// Forget the panel device's FCM token before following the logout link, so the
// row in user_fcm_tokens is removed instead of lingering as a stale subscription.
document.addEventListener('click', async function (e) {
    const link = e.target.closest('.js-panel-logout');
    if (!link) return;
    if (panel !== 'admin' && panel !== 'seller') return;
    ...
    e.preventDefault();
    try {
        await axios.delete(`${base_url}/${panel}/devices`, {
        ...
        console.warn('Panel device forget failed:', err);
    }
