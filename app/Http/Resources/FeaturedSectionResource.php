...
class FeaturedSectionResource extends JsonResource
{
    ...
    public function toArray($request): array
    {
        ...
        if ($latitude && $longitude && $zoneInfo) {
            ...
            $products = $this->products()->take($productsLimit)->get();
            $productsCount = $this->getProductsQuery()->count();
        }

        $products = Product::markSponsored($products);
        ...
        return [
            ...
            'selected_products' => $this->whenLoaded('products', fn () => $this->products->map(fn ($product) => [
                'id' => $product->id,
                'title' => $product->title,
            ])->values()),
            'uses_manual_products' => $this->section_type === FeaturedSectionTypeEnum::CUSTOM_PRODUCTS(),
            'scope_category' => new CategoryResource($this->whenLoaded('scopeCategory')),
            'zones' => $this->whenLoaded('zones', fn() => $this->zones->map(fn($z) => ['id' => $z->id, 'name' => $z->name])->values()),
