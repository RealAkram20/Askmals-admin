...
class NotificationController extends Controller
{
    ...
    {
        try {
            $notification = $this->panelNotificationQuery()
                ->with('notifiable')
                ->findOrFail($id);
    ...
    {
        try {
            $notification = $this->panelNotificationQuery()->findOrFail($id);
    ...
    public function getUserNotifications(Request $request): JsonResponse
    {
        try {
            ...
            $result = $this->notificationService->getUserNotifications($userId, $perPage, $this->panelRoleType());
    ...
    {
        try {
            $count = $this->notificationService->getUnreadCount((int) auth()->id(), $this->panelRoleType());
    ...
    private function panelRoleType(): ?NotificationRoleTypeEnum
    {
        return match ($this->getPanel()) {
            'admin' => NotificationRoleTypeEnum::ADMIN,
            'seller' => NotificationRoleTypeEnum::SELLER,
            default => null,
        };
    }
