...
class SubscriptionPlanController extends Controller
{
    ...
    public function datatable(Request $request): JsonResponse
    {
        ...
        $data = $plans->map(function ($plan) {
            ...
            foreach (SubscriptionPlanKeyEnum::values() as $key) {
                if (SubscriptionPlanKeyEnum::isBoolean($key)) {
                    $boolVal = (int) ($limitMap[$key] ?? 0);
                    $label = $boolVal ? __('labels.enabled') : __('labels.disabled');
                } else {
                    $label = $limitMap[$key] ?? __('labels.unlimited');
                }
                $configHtml .= '<li class="">' . e(Str::replace("_", " ", $key)) . ': ' . e($label) . '</li>';
            }
            $configHtml .= '</ol>';
