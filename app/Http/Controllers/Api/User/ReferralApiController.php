...
use App\Http\Requests\User\SubmitReferralRequest;
...
class ReferralApiController extends Controller
{
    ...
    /**
     * Submit (or dismiss) the post-registration referral prompt.
     *
     * Replaces the legacy "ask for friends_code during mobile-verify modal"
     * flow that the app store rejected for Apple sign-in. The mobile app
     * shows a dedicated referral page after signup and POSTs once: send a
     * non-empty `friends_code` to apply it, or omit / send empty to dismiss
     * the prompt forever. The endpoint is single-shot — once a customer has
     * either applied a code or skipped, subsequent calls return 422.
     *
     * @param  SubmitReferralRequest  $request
     * @return JsonResponse
     */
    public function submit(SubmitReferralRequest $request): JsonResponse
    {
        try {
            $user = Auth::user();
            ...
            $result = $this->referralService->submitForUser(
                $user,
                $validated['friends_code'] ?? null
            );
            ...
            Log::error('ReferralApiController::submit — ' . $e->getMessage());
            return ApiResponseType::sendJsonResponse(
                false,
                'labels.something_went_wrong',
                ['error' => $e->getMessage()]
