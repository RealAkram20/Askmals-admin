...
class OtpApiController extends Controller
{
    ...
    public function verifyOtp(VerifyOtpRequest $request): JsonResponse
    {
        try {
            ...
            if ($authedUser && $this->isDemoModeEnabled()) {
                return ApiResponseType::sendJsonResponse(
                    false,
            ...
            $isRegister = $resolved['status'] === 'created';
            $messageKey = $isRegister
                ? 'labels.registration_successful'
                : 'labels.verified_successfully';

            return $this->tokenResponse($resolved['user'], $messageKey, $sanitizedMobile, $isRegister);
        } catch (\Throwable $e) {
     ...
     */
    protected function tokenResponse(User $user, string $messageKey, string $tokenName, bool $isRegister = false): JsonResponse
    {
