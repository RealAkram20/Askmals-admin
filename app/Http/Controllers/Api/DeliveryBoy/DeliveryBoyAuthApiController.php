...
class DeliveryBoyAuthApiController extends Controller
{
    ...
    public function login(Request $request): JsonResponse
    {
        try {
            ...
            // Admin block — refuse login outright. Surface the reason so the
            // rider knows why and can contact support, but do NOT issue a token.
            if ($deliveryBoy->is_blocked) {
                return ApiResponseType::sendJsonResponse(
                    success: false,
                    message: __('labels.delivery_boy_account_blocked'),
                    data: [
                        'blocked_reason' => $deliveryBoy->blocked_reason,
                        'blocked_at' => $deliveryBoy->blocked_at,
                    ]
                );
            }
