...
class SellerAddonGroupApiController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        try {
            ...
            $user = auth()->user();
            // Resolve the seller robustly: first by direct ownership, then via the accessor (which also checks pivot)
            $seller = null;
            if ($user) {
                $seller = \App\Models\Seller::where('user_id', $user->id)->first();
                if (!$seller) {
                    // fallback to accessor which also inspects SellerUser pivot
                    $seller = $user->seller();
                }
            }
            if (!$seller) {
