...
class DeliveryZoneService
{
    ...
    public static function isPointInPolygon(DeliveryZone $zone, float $latitude, float $longitude): bool
    {
        ...
        if (is_string($polygon)) {
            ...
            $polygon = is_array($decoded) ? $decoded : null;
        }
        ...
        } elseif (isset($polygon[0]['lat']) || isset($polygon[0]['latitude']) || isset($polygon[0][0])) {
            $coordinates = $polygon;
        } else {
            return false;
        }
     ...
     */
    public static function calculateDeliveryRoute(float $customerLat, float $customerLng, array $storeIds, $order = null, ?float $startLat = null, ?float $startLng = null): array
    {
        if (empty($storeIds)) {
            return [
                'total_distance' => 0,
        ...
        $originLat = $startLat ?? $customerLat;
        $originLng = $startLng ?? $customerLng;

        $stores = self::getStoresForRoute($originLat, $originLng, $storeIds);
    ...
    /**
     * Fetch stores for the route ordered by distance from the origin point.
     */
    private static function getStoresForRoute(float $originLat, float $originLng, array $storeIds): \Illuminate\Support\Collection
    {
        return Store::whereIn('id', $storeIds)
            ->select([
                'id', 'name', 'latitude', 'longitude', 'address', 'city', 'landmark', 'state', 'zipcode', 'country', 'country_code',
                DB::raw("(6371 * acos( cos(radians($originLat)) * cos(radians(latitude)) * cos(radians(longitude) - radians($originLng)) + sin(radians($originLat)) * sin(radians(latitude)) )) AS distance_from_customer")
            ])
    ...
    /**
     * Get delivery boys within map viewport (bounding box).
     */
    public static function getRidersByBounds(
        float $neLat,
        float $neLng,
        float $swLat,
        float $swLng,
        ...
        if (
            !self::validateCoordinates($neLat, $neLng) ||
            !self::validateCoordinates($swLat, $swLng)
        ) {
            return [
                'success' => false,
                'message' => __('labels.invalid_coordinates'),
        ...
        $locations = DeliveryBoyLocation::whereBetween('latitude', [$swLat, $neLat])
            ->whereBetween('longitude', [$swLng, $neLng])
            ->with([
                'deliveryBoy' => fn ($q) => $q->whereNull('deleted_at'),
                'deliveryBoy.user:id,name,email,mobile',
            ])
            ->limit($limit)
            ->get();

        $riders = $locations
            ->filter(fn ($loc) => $loc->deliveryBoy !== null)
            ->map(function ($loc) use ($activeAssignmentStatuses) {
                $db = $loc->deliveryBoy;

                $hasActiveAssignment = $db->assignments()
                    ->whereIn('status', $activeAssignmentStatuses)
                    ->exists();

                if ($hasActiveAssignment) {
                    $deliveryStatus = 'on_delivery';
                } elseif (!$db->is_blocked && $db->verification_status === DeliveryBoyVerificationStatusEnum::APPROVED()) {
                ...
                return [
                    ...
                    'phone'           => $db->user->mobile ?? null,
                    'latitude'        => (float) $loc->latitude,
                    'longitude'       => (float) $loc->longitude,
                    'delivery_status' => $deliveryStatus,
                    'is_blocked'      => (bool) $db->is_blocked,
