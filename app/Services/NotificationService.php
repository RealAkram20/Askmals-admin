...
class NotificationService
{
    ...
    public function createNotification(array $data): Notification
    {
        try {
            ...
            $roleType = $data['role_type'] instanceof NotificationRoleTypeEnum
                ? $data['role_type']
                : NotificationRoleTypeEnum::fromSentTo($data['sent_to'] ?? null);
     ...
     */
    public function getUserNotifications(int $userId, int $perPage = 15, ?NotificationRoleTypeEnum $roleType = null): array
    {
        $notifications = $this->queryForUser($userId, $roleType)
            ->with('notifiable')
        ...
        return [
            ...
            'unread_count' => $this->getUnreadCount($userId, $roleType),
            'pagination' => [
     ...
     */
    public function getUnreadCount(int $userId, ?NotificationRoleTypeEnum $roleType = null): int
    {
        return $this->queryForUser($userId, $roleType)
            ->whereNull('read_at')
            ->count();
    }
     ...
     */
    public function markAllAsRead(int $userId, ?NotificationRoleTypeEnum $roleType = null): bool
    {
        try {
            DB::beginTransaction();

            $this->queryForUser($userId, $roleType)
                ->whereNull('read_at')
                ->update(['read_at' => now()]);
    ...
    public function getHeaderNotifications(int $userId, ?string $sentTo = null): array
    {
        $query = $this->queryForUser(
            $userId,
            $sentTo ? NotificationRoleTypeEnum::fromSentTo($sentTo) : null
        );
        if (! empty($sentTo)) {
            $query->where('data->sent_to', $sentTo);
        }
    ...
    public function queryForUser(int $userId, ?NotificationRoleTypeEnum $roleType = null): Builder
    {
