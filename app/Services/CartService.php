...
class CartService
{
    ...
    {
        try {
            return DB::transaction(function () use ($addressId, $isRushDelivery, $latitude, $longitude, $promoCode, $useWallet, $user) {
                $cart = $this->getUserCart($user);
                ...
                $cart = $this->calculateCartPaymentSummary(
                    cart: $cart,
                    latitude: $latitude,
                    longitude: $longitude,
    ...
    {
        if (!$isRushDelivery && !empty($freeDeliveryAmount) && $itemsTotal >= $freeDeliveryAmount) {
            return 0;
        }
    ...
    {
        try {
            return DB::transaction(function () use ($addons, $cartItemId, $quantity, $user) {
                $cartItem = CartItem::whereHas('cart', function ($query) use ($user) {
                    $query->where('user_id', $user->id);
                })->find($cartItemId);
