...
class SubscriptionUsageService
{
    ...
    public function getSellerPlanLimits(int $sellerId): array
    {
        ...
        foreach (SubscriptionPlanKeyEnum::values() as $key) {
            if (!array_key_exists($key, $limits)) {
                $limits[$key] = SubscriptionPlanKeyEnum::isBoolean($key) ? 0 : -1;
            }
        }
    ...
    /**
     * Check if a seller's active plan includes POS access.
     *
     * Bypassed entirely when subscription gating is disabled globally (or when
     * the system is running in single-vendor mode) — POS is then available to
     * every seller regardless of plan, matching SubscriptionLimitGuard semantics.
     */
    public function hasPosAccess(int $sellerId): bool
    {
        if (!Setting::isSystemVendorTypeMultiple() || !Setting::isSubscriptionEnabled()) {
            return true;
        }

        $limit = $this->getLimit($sellerId, SubscriptionPlanKeyEnum::POS_ACCESS());
