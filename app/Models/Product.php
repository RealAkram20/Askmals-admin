...
{
    use HasSeoMetadata, InteractsWithMedia, SoftDeletes;

    protected function seoFallbackDescription(): ?string
    {
        $value = $this->short_description ?? null;
    ...
    protected function seoFallbackKeywords(): array
    {
        ...
        if (is_array($tags)) {
            $tags = array_values(array_filter(array_map('trim', $tags), fn ($v) => is_string($v) && $v !== ''));
            if (! empty($tags)) {
    ...
    public function scopeApplyProductFilter(Builder $query, ?string $filter): Builder
    {
        ...
        try {
            ...
            } elseif ($filter === ProductFilterEnum::HAS_BADGE()) {
    ...
    public function setVerificationStatus($value): void
    {
    ...
    {
        return $this->belongsToMany(FeaturedSection::class, 'featured_section_product')
            ->withTimestamps();
    }
     ...
     */
    public function getFormattedVariantAttributes(): array
    {
        ...
        switch ($sort) {
            case 'price_asc':
                ...
                $priceSub = DB::table('product_variants')
                    ...
                    ->where(fn ($q) => $q->where('store_product_variants.special_price', '>', 0)->orWhere('store_product_variants.price', '>', 0))
            ...
            case 'has_badge':
                $query->whereNotNull('badge_id')->orderBy('id', 'desc');
                break;
    ...
    private static function applyBadgeFilter(Builder $query, array $filter): void
    {
        if (empty($filter['badge_id'])) {
            return;
        }

        $query->where('badge_id', (int) $filter['badge_id']);
    }
    ...
    public static function getProductsByLocation(float $latitude, float $longitude, int $perPage = 15, array $filter = []): LengthAwarePaginator
    {
        ...
        $baseQuery = self::scopeByLocation(zoneInfo: $zoneInfo, query: self::query(), filter: $filter);
    ...
    /**
     * Tag products that have active running campaigns with sponsored fields.
     * When sort is 'relevance' (or null), sponsored products are moved to the top.
     *
     * @param  \Illuminate\Support\Collection|AbstractPaginator|self  $products
     * @param  string|null  $sort  Current sort filter — sponsored-first only applies on 'relevance' or null.
     * @return \Illuminate\Support\Collection|AbstractPaginator|self
     */
    public static function markSponsored($products, ?string $sort = null)
    {
        if ($products instanceof Relation || $products instanceof Builder) {
            $products = $products->get();
        }

        $isSingle = $products instanceof self;
        $isPaginator = $products instanceof AbstractPaginator;
        $items = $isSingle ? collect([$products]) : (
            $isPaginator ? $products->getCollection() : $products
        );

        $productIds = $items->pluck('id')->filter()->toArray();

        if (empty($productIds)) {
            return $products;
        }

        $adServingService = app(AdServingService::class);
        $campaigns = $adServingService->findCampaignsForProducts($productIds, AdPlacementEnum::SEARCH());
        ...
        $visitorKey = hash('sha256', $ip . '|' . $userAgent . '|' . $userId);

        foreach ($items as $product) {
            $campaign = $campaigns->get($product->id);
            if ($campaign) {
                $product->is_sponsored = true;
                $product->campaign_id  = $campaign['campaign_id'];
                $product->visitor_key  = $visitorKey;
            }
        }
        ...
        // Only promote sponsored to top when no explicit sort is applied
        if ($sort !== null && $sort !== '' && $sort !== 'relevance' && $sort !== 'default') {
            return $products;
        }

        $sorted = $items->sortByDesc(fn ($p) => (bool) ($p->is_sponsored ?? false))->values();
