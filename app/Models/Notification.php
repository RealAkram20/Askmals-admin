...
class Notification extends BaseDatabaseNotification
{
    ...
    {
        $enum = $roleType instanceof NotificationRoleTypeEnum
            ? $roleType
            : NotificationRoleTypeEnum::from($roleType);

        return $query->where(function ($scopedQuery) use ($enum) {
            $scopedQuery->where('role_type', $enum->value)
                ->orWhere(function ($legacyQuery) use ($enum) {
                    $legacyQuery->whereNull('role_type')
                        ->whereIn('data->sent_to', $enum->legacySentToValues());
                });
        });
    }
    ...
    public function getResolvedRoleTypeAttribute(): ?string
    {
        return $this->role_type?->value
            ?? NotificationRoleTypeEnum::fromSentTo($this->sent_to)?->value;
    }
