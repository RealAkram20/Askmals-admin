...
class OrderItem extends Model implements HasMedia
{
    ...
    public static function getStatusHierarchy(): array
    {
        return [
            ...
            // DELIVERY_FAILED sits at the same step as DELIVERED — both represent the
            // delivery attempt completing. Failures funnel into RETURNING_TO_STORE next.
            OrderItemStatusEnum::DELIVERY_FAILED() => 6,
            OrderItemStatusEnum::RETURNED() => 7,
            // Terminal or transient-terminal-path statuses don't follow forward hierarchy.
            OrderItemStatusEnum::RETURNING_TO_STORE() => -1,
    ...
    public function isValidStatusTransition(string $oldStatus, string $newStatus): bool
    {
        ...
        // RETURNING_TO_STORE is a transient terminal-path state. The seller must be
        // able to confirm receipt and finalise the item to CANCELLED — handle that
        // before the generic terminal guard below.
        if ($oldStatus === OrderItemStatusEnum::RETURNING_TO_STORE()) {
            return $newStatus === OrderItemStatusEnum::CANCELLED();
        }
        ...
        // Once a delivery attempt has failed, items must travel through the
        // return-to-store flow — direct re-delivery isn't valid.
        if ($oldStatus === OrderItemStatusEnum::DELIVERY_FAILED() && $newStatus === OrderItemStatusEnum::DELIVERED()) {
            return false;
        }
